...
| `architect.html` | Architect Walkthrough product page. **Green-themed**: the page overrides the ramp tokens at `body.arch` level with the app's pine ramp (brand `#1B4D3E`, mint accent `#9FD6BF`); everything else reuses the global kit. Screens in `assets/screens/architect/`, demo video and poster in `assets/video/`. App is **live on the App Store** (released 2026-07-24): both launch stamps say "Now on the App Store" and both CTAs are the store badge linking `https://apps.apple.com/app/id6787164107`. Not linked from the homepage (Josh's call to keep builder and architect surfaces separate; reachable by direct URL, sitemap and llms.txt). |
